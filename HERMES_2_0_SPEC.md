...
| `implementation_hash` | Yes | No | Sha256 | N/A | Adapter commitment. |
...
For normalized canary sequence C with length L and candidate sequence T, `min_match_length = max(8, ceil(0.50*L))` when `L < 16`, otherwise `max(8, ceil(0.25*L))`. A contiguous substring of T that is also a contiguous substring of C and has at least that length is a partial match. Offsets are normalized Unicode code-point offsets, half-open `[start,end)`. The ratio is `matched_code_points / L`.

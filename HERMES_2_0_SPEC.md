...
For normalized canary sequence C with length L and candidate sequence T, `min_match_length = max(8, ceil_div(L, 2))` when `L < 16`, otherwise `max(8, ceil_div(L, 4))`, where `ceil_div(a, b) = floor((a + b - 1) / b)` for positive integers `a` and `b`. No floating-point arithmetic is used in this calculation. A contiguous substring of T that is also a contiguous substring of C and has at least that length is a partial match. Offsets are expressed in the following coordinate system: for `exact` semantics, offsets are raw decoded code-point positions in the detector input representation before normalization; for all other semantics (`normalized`, `encoded`, `partial`, `declared_transform`), offsets are normalized Unicode code-point positions in the post-normalization candidate sequence. `normalized_hash` in all cases is SHA-256 of the UTF-8 encoding of the normalized matched segment. The ratio is `matched_code_points / L` using integer division where both values are `CodePointCount` integers.
...
| `adapter_hash` | Yes | No | Sha256 | SHA-256 of canonical JCS bytes of the AdapterImplementationDescriptor defined in Section 14 | Adapter commitment. Identical semantics to ControlAdapter.implementation_hash. |
...
| `optional_detector_error OR optional_detector_not_run OR optional_canary_leakage OR policy_check_review OR policy_check_fail OR human_review_requested` | REVIEW | Corresponding review reason |
